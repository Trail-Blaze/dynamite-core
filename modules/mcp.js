      ...
      function (req, res) {
         ...
         const getOrCreateProfile = (profileId) => {
            ...
            if (!profileData) {
               ...
               if (!fs.existsSync(`./home/${accountId}/profile`)) {
                  try {
                     console.log(
                        `NO PROFILE FOUND! Creating one in /home/${accountId} dir`
                     );
         ...
         const checkValidProfileID = (...validProfileIds) =>
            checkValidProfileID0(command, profileId, ...validProfileIds);
         //profile commands
         switch (command) {
            ...
            case "PurchaseCatalogEntry":
               ...
               const shop = require("./lib/shop.json");
               ...
               for (lootResultEntry of lootResult) {
                  Profile.addItem(
                     grantProfile.profileData,
                     lootResultEntry.itemGuid,
                     {
               ...
               if (grantProfile.response.profileChanges.length > 0) {
                  ...
                  response.profileCommandRevision =
                     grantProfile.profileData.commandRevision || 1;
                  Profile.saveProfile(
                     accountId,
                     grantToProfileId,
                     grantProfile.profileData
            ...
            case "SetAffiliateName":
               ...
               Profile.modifyStat(
                  profileData,
                  "mtx_affiliate",
                  force ? "DynamiteKittyCat_UwU" : req.body.affiliateName,
                  profileChanges
               );
               Profile.modifyStat(
                  profileData,
            ...
            case "SetCosmeticLockerBanner": {
               ...
               if (
                  ...
                  item.attributes.banner_icon_template !=
            ...
            case "SetCosmeticLockerSlot": {
               ...
               if (!lockerSlot) {
                  const capacity =
                     req.body.category == "Dance"
                        ? 6
                        : req.body.category == "ItemWrap"
                        ? 7
                        : 1;
               ...
               switch (req.body.slotName) {
                  ...
                  case "ItemWrap":
                     ...
                     if (req.body.indexWithinSlot == -1) {
                        ...
                        arr[req.body.indexWithinSlot || 0] =
                           req.body.itemToSlot;
                     }
            ...
            case "SetItemFavoriteStatus":
               checkValidProfileID("campaign", "athena");
               if (
                  typeof req.body.bFavorite === "boolean" &&
                  profileData.items[req.body.targetItemId].attributes
                     .favorite != req.body.bFavorite
               ) {
                  Profile.changeItemAttribute(
                     profileData,
                     req.body.targetItemId,
                     "favorite",
                     req.body.bFavorite,
                     profileChanges
                  );
            ...
            case "SetItemFavoriteStatusBatch":
               ...
               req.body.itemIds.forEach((itemId, index) => {
                  if (
                     ...
                     typeof req.body.itemFavStatus[index] === "boolean"
                  ) {
                     Profile.changeItemAttribute(
                        profileData,
                        itemId,
                        "favorite",
                        req.body.itemFavStatus[index]
                     ),
                        profileChanges;
                  }
               });
            ...
            case "SetReceiveGiftsEnabled":
               ...
               if (typeof req.body.bReceiveGifts === "boolean") {
                  Profile.modifyStat(
                     profileData,
                     "allowed_to_receive_gifts",
                     req.body.bReceiveGifts,
                     profileChanges
                  );
               }
               break;
...
function notfound() {
   throw new ApiException(
      errors.com.dynamite.fortnite.item_not_found
   ).withMessage("Locker item {0} not found!", req.body.lockerItem);
}
