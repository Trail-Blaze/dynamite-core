...
module.exports = {
    ...
    try {
      return JSON.parse(
        fs.readFileSync(
          path.join(
            __dirname,
            `../home/default/${accountId}/profile/profile_${profileId}.json`
          ),
          "utf8"
        )
      );
  ...
  readProfileTemplate(profileId) {
    // console.log(`../home/default/profile/profile_${profileId}.json`);
    try {
      return JSON.parse(
        fs.readFileSync(
          path.join(
            __dirname,
            `../home/default/profile/profile_${profileId}.json`
          ),
          "utf8"
        )
      );
  ...
  saveProfile(accountId, profileId, data) {
    fs.writeFileSync(
      path.join(
        __dirname,
        `../home/${accountId}/profile/profile_${profileId}.json`
      ),
